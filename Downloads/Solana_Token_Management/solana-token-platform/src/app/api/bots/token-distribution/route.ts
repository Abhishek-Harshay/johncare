...
import { tokenDistributionService } from '@/lib/services/bots/tokenDistribution'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionId && action === 'export') {
      ...
      const csvData = tokenDistributionService.exportTransactions(sessionId)
      if (!csvData) {
      ...
      return new NextResponse(csvData, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="token_distribution_${sessionId}.csv"`
        }
      })
    }
    ...
    if (sessionId) {
      ...
      const session = tokenDistributionService.getSession(sessionId)
      if (!session) {
        return NextResponse.json({
          success: false,
    ...
    // Get all sessions
    const sessions = tokenDistributionService.getAllSessions()
    ...
    console.error('Token Distribution API Error (GET):', error)
    return NextResponse.json({
      success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'start': {
        ...
        if (!config.decimals || config.decimals < 0) {
          return NextResponse.json({
            success: false,
            message: 'Token decimals is required and must be >= 0'
          }, { status: 400 })
        }

        // Start new distribution session
        const result = await tokenDistributionService.startDistribution({
          ...
          decimals: parseInt(config.decimals)
        })
      ...
      case 'pause': {
        ...
        const result = await tokenDistributionService.pauseSession(sessionId)
        return NextResponse.json(result, {
          status: result.success ? 200 : 400
        })
      }
      ...
      case 'resume': {
        ...
        const result = await tokenDistributionService.resumeSession(sessionId)
        return NextResponse.json(result, {
          status: result.success ? 200 : 400
        })
      }
      ...
      case 'stop': {
        ...
        const result = await tokenDistributionService.stopSession(sessionId)
        return NextResponse.json(result, {
          status: result.success ? 200 : 400
        })
      }
      ...
      case 'cleanup': {
        const cleaned = tokenDistributionService.cleanup()
        return NextResponse.json({
          success: true,
    ...
    console.error('Token Distribution API Error (POST):', error)
    return NextResponse.json({
      success: false,
